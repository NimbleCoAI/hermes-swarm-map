...
describe('HarnessService.duplicate', () => {
  ...
  it('duplicates a harness overlay with a new name', () => {
    ...
    const result = service.duplicateOverlay('h_personal', 'personal-v2')
  ...
  it('returns undefined when source overlay does not exist', () => {
    storage.write('harnesses.json', [])
    const result = service.duplicateOverlay('h_nonexistent', 'new-name')
    expect(result).toBeUndefined()
  })

  it('generates unique id from name', () => {
    storage.write('harnesses.json', [
      { id: 'h_personal', name: 'personal', tier: 'team' },
    ])
    const result = service.duplicateOverlay('h_personal', 'my-new-agent')
    expect(result!.id).toBe('h_my_new_agent')
  })

  it('logs duplication to audit', () => {
    storage.write('harnesses.json', [
      { id: 'h_personal', name: 'personal', tier: 'individual' },
    ])
    service.duplicateOverlay('h_personal', 'personal-copy')
