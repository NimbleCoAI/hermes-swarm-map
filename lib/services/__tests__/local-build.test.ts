...
describe('Local build compose generation', () => {
  ...
  it('generates compose with build directive when useLocalBuild is true', () => {
    ...
    const result = service.createOverlay({ name: 'build-test' })
  ...
  it('generates compose with image directive when useLocalBuild is false', () => {
    config.updateSettings({
      useLocalBuild: false,
      hermesDir: fakeHermesDir,
    })
    ...
    storage.write('harnesses.json', [])
    const result = service.createOverlay({ name: 'image-test' })

    const composeFile = result.composeFile!
    expect(fs.existsSync(composeFile)).toBe(true)
    const content = fs.readFileSync(composeFile, 'utf-8')

    expect(content).toContain('image: nousresearch/hermes-agent:latest')
    expect(content).not.toContain('build:')
  })

  it('falls back to image when hermesDir has no Dockerfile', () => {
    ...
    storage.write('harnesses.json', [])
    const result = service.createOverlay({ name: 'fallback-test' })
  ...
  it('uses local build for duplicated harnesses too', () => {
    config.updateSettings({
      useLocalBuild: true,
      hermesDir: fakeHermesDir,
    })
    ...
    const source = service.createOverlay({ name: 'source-agent' })

    // Duplicate it
    const duplicate = service.duplicateOverlay(source.id!, 'cloned-agent')
