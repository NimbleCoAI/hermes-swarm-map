...
import type { Settings, Harness, Model } from '@/lib/types'
...
export default function SettingsPage() {
  ...
  const { data: models, loading: mLoading } = useApi<Model[]>('/api/models')
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Model Gating */}
      <section>
        <h3 className="text-base font-medium mb-3">Model Access Tiers</h3>
        {mLoading && <p className="text-muted-foreground">Loading...</p>}
        {!mLoading && models && models.length > 0 && (
                ...
                {models.map((m) => (
                  <tr key={m.id} className="border-b border-[var(--border)] last:border-0">
                    ...
                    <td className="px-4 py-3">
                      <CostBadge costClass={m.costClass} />
                    </td>
                    <td className="px-4 py-3">
        ...
        {!mLoading && (!models || models.length === 0) && (
          <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-6 text-center space-y-2">
            <p className="text-sm text-muted-foreground">
              No models registered yet. Models are discovered from harness configurations.
            </p>
            <p className="text-xs text-muted-foreground">
              Per-tier model gating with admin controls is planned for v2.
            </p>
          </div>
        )}
      </section>
    ...
    <div className="flex items-center justify-between py-1.5 px-2 rounded-md hover:bg-muted/30 transition-colors">
      ...
      <div className="flex items-center gap-1">
        <Button
          ...
          size="xs"
          onClick={restart}
          disabled={acting || harness.status === 'stopped'}
        >
          Restart
        </Button>
        <Button
...
function CostBadge({ costClass }: { costClass: Model['costClass'] }) {
  const styles: Record<Model['costClass'], string> = {
  ...
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${styles[costClass]}`}>
      {costClass}
    </span>
  )
}
